...
const GuestOverview: React.FC = () => {
  ...
  useEffect(() => {
    getGuestDashboard()
      .then(setData)
      .catch((err) => setError(err?.error || err?.message || 'Failed to load dashboard data.'))
  }, [])
    ...
    <>
      ...
      <div className="ea-dash-grid">
        <div className="ea-dash-card">
          ...
          {la?.active ? (
            <>
              <p style={{ color: '#2ecc71', fontSize: 14, margin: '0 0 8px' }}>Access Active - {la.lounge_name || 'Lounge'}</p>
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 13, margin: 0 }}>
                Valid until {la.valid_until ? new Date(la.valid_until).toLocaleTimeString() : '-'}
              </p>
            </>
          ) : (
            <>
